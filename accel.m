%% delete this later; it's only setup
close("all")
clear

% check if run from correct path:
[~, lastdir, ~] = fileparts(pwd);
if (~strcmp(lastdir, "AE4301P")) || (exist("./F16Sim/") ~= 7)
...
% provides the all-important SS_long matrix after using the modified 
% simulink model
run lin_accel.m
...
sprintf("a_n depends on %s", names(an_dependencies).join(", "))
...
for xa = xa_sweep
    ...
    zeros_el2an_sweep{string(xa), 2:end} = zpk(el2an).Z{1}';
...
export_fig Accel_results/xa_sweep.eps -painters


% output table as well
writetable(zeros_el2an_sweep, 'Accel_results/Zeros_Sweep.csv',...
                              'WriteRowNames', false)
zeros_el2an_sweep
